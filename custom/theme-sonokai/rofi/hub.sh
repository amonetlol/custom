...
THEME_DIR="${HOME}/.config/custom/theme-sonokai"
ROFI_DIR="$THEME_DIR/rofi"
WAYBAR_DIR="$THEME_DIR/waybar"
FOOT_INI="${HOME}/.config/custom/foot/foot.ini"
IDLE_PID_FILE="${XDG_RUNTIME_DIR:-/tmp}/theme-sonokai-idle-inhibit.pid"
...
case "$1" in
  ...
  --idle-on)
    ...
    if command -v systemd-inhibit >/dev/null; then
      systemd-inhibit --what=idle:sleep --who=theme-sonokai --why="stay awake" \
        sleep infinity &
      echo $! >"$IDLE_PID_FILE"
