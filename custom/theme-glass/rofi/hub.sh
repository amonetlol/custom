...
THEME_DIR="${HOME}/.config/custom/theme-glass"
ROFI_DIR="$THEME_DIR/rofi"
WAYBAR_DIR="$THEME_DIR/waybar"
FOOT_INI="$THEME_DIR/foot/foot.ini"
IDLE_PID_FILE="${XDG_RUNTIME_DIR:-/tmp}/theme-glass-idle-inhibit.pid"
...
case "$1" in
  ...
  --idle-on)
    ...
    if command -v systemd-inhibit >/dev/null; then
      systemd-inhibit --what=idle:sleep --who=theme-glass --why="stay awake" \
        sleep infinity &
      echo $! >"$IDLE_PID_FILE"
